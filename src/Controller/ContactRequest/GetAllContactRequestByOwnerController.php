...
use App\MiMascota\ContactRequests\Application\GetAllContactRequestByOwner;
...
class GetAllContactRequestByOwnerController extends AbstractController
{
    use ApiResponseTrait, AuthorizationCheckerTrait;
    #[Route('/contactRequest/get/owner/{id}', name: 'get_contact_requests_owner', methods: ['GET'])]
    public function __invoke(Request $request, string $id, GetAllContactRequestByOwner $byOwner) : Response
    {
        try {
            ...
            $response = $byOwner->__invoke(new GetAllContactRequestByUserQuery($id));
